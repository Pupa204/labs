...
class Lesson:
    ...
    def initLesson(cls, line):
        ...
        if not line.startswith('"'):
            raise Exception("Неправильно заполнена аудитория") 
        line = line[1:]
        nameAudit = line[:line.find('"')]
        if nameAudit.strip() == "":
            raise Exception("Не заполнена аудитория")

        line = line[line.find('"') + 1:].strip()
        if not line.startswith('"'):
            raise Exception("Неправильно заполнено имя преподавателя")
        line = line[1:]
        namePrepod = line[:line.find('"')]
        if namePrepod.strip() == "":
            raise Exception("Не заполнено имя преподавателя")
